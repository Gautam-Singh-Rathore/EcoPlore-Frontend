...
import { Leaf, BarChart3, Package, Shield } from "lucide-react";
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
               <img className=" h-10 w-28 md:h-13 md:w-44"   src={logo} alt="GreenPlore" width={50} height={50}  />
...
<div className="flex items-center gap-3">
    ...
    Logout
      ...
      <section className="py-20">
        <div className="container mx-auto px-6 text-center">
          ...
          >
            <Link to="/dashboard">
              <Button size="lg" className="bg-green-600 text-white hover:bg-green-700 cursor-pointer ">
                Start Selling
              </Button>
            </Link>
  ...
  <div className="container mx-auto px-6">
    ...
    <p className="text-center text-gray-600 mb-12 max-w-2xl mx-auto">
      Access all the tools you need to manage your sustainable business from one powerful dashboard
    </p>
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      {features.map((item, index) => (
        <Link key={index} to={item.link} className="group">
          <Card className="h-full shadow hover:shadow-lg transition-all duration-300 hover:scale-105 animate-scale-in" style={{ animationDelay: `${index * 100}ms` }}>
            <CardContent className="p-4 text-center"> {/* Reduced padding */}
              <div className={`${item.color} w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform duration-200`}>
                <item.icon className="h-6 w-6 text-white" /> {/* Smaller icon */}
              </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {[
        ...
        },
        {
          ...
          description:
            "Monitor performance with detailed analytics and insights.",
        },
        {
          icon: Shield,
